    ...
    <div className="shadow-md w-full fixed top-0 left-0 z-10">
      <div className="md:flex items-center justify-between bg-slate-900 py-4 md:px-14 px-7">
        <div className="flex items-center font-bold text-2xl cursor-pointer font-[Roboto] flex-shrink-0">
          <Link to="/">
            <img className="md:h-14 h-12 object-cover" src={logo} alt="logo" />
          </Link>
        </div>
        ...
        <ul
          className={`md:flex flex-shrink-0 md:items-center md:pb-0 pb-12 absolute md:static md:bg-slate-900 bg-slate-900 md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 ${
            open
              ? "top-20 opacity-100 transition-all duration-300 ease-in"
            ...
            ? loggedInLinks.map((link) => (
                <li key={link.name} className="text-xl md:my-0 my-4">
                  ...
                  >
                    {link.name === "Log Out" ? (
                      <button
                        className="block text-slate-50 hover:bg-slate-800 rounded transition duration-300 ease-in-out"
                        onClick={() => {
