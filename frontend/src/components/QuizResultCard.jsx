    ...
    <div className="bg-slate-200 px-4 py-5 sm:px-6 rounded-md shadow-md">
      <div className="-ml-4 -mt-2 flex flex-wrap items-center justify-between sm:flex-nowrap">
        <div className="flex-row ml-4 mt-2">
          <Link to={`/quiz-results-detail/${candidate._id}/${candidate.quizTaken[0].quizId}`}>
            <h3 className="text-lg text-left font-semibold leading-6 text-gray-900 hover:underline">{candidate.name}</h3>
          </Link>
          <p className='text-base'>{formatDate(candidate.quizTaken[0].takenAt)}</p>
        </div>
