...
const AddQuestion = () => {
  ...
  useEffect(() => {
    questionType === 'multiple-choice' ? setMultipleChoiceVisible(true) : setMultipleChoiceVisible(false);
    questionType === 'true-false' ? setTrueFalseVisible(true) : setTrueFalseVisible(false);
    questionType === 'short-answer' ? setShortAnswerVisible(true) : setShortAnswerVisible(false);
    questionType === 'select-all' ? setSelectAllVisible(true) : setSelectAllVisible(false);
  ...
  return (
    <div className='w-full py-16 px-4 shadow-lg bg-slate-200 mt-20'>
      <div className='max-w-[1000px] mx-auto'>
        <h1 className='md:text-4xl text-3xl font-semibold text-slate-800 mt-2 mb-4 md:text-left text-center'>Add Question</h1>
        <button
        ...
        <div className='flex flex-col md:flex-row md:space-x-8 space-y-4 md:space-y-0 mt-10'>
          <div className='bg-slate-100 rounded-md shadow-lg py-8 w-full text-lg'>
            <div className='text-center'>
              <div>
                <h2 className='text-2xl font-semibold text-slate-800'>Question Options</h2>
                <div className='max-w-[50%] m-auto mb-6'>
                  <label htmlFor="location" className="block text-lg font-medium leading-6 text-slate-900 text-left mt-4">
                    Question Type
                  </label>
                  <select
                    id="questionType"
