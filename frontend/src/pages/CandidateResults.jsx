...
const CandidateResult = () => {
  ...
  useEffect(() => {
    const fetchCandidateData = async () => {
      ...
      try {
        const { data } = await axios.get(`http://localhost:5000/candidate/${quizId}/taken`, config);
  ...
  return (
    <div className='w-full min-h-screen py-16 px-4 shadow-lg bg-slate-200 mt-20'>
      <div className='max-w-[1250px] mx-auto'>
        <h1 className='md:text-4xl text-3xl font-semibold text-slate-800 mt-2 mb-4 md:text-left text-center'>{quiz.title} Results | {candidate.name}</h1>
        <div className='flex flex-col md:flex-row md:space-x-8 space-y-4 md:space-y-0 mt-10'>
          <div className='bg-slate-100 rounded-md shadow-lg py-8 w-full text-lg'>
            <div>
              <h1 className='text-2xl font-semibold text-slate-800 mb-4 px-10'>Quiz Score: {score}</h1>
              <div className="relative mt-5">
                <div className="absolute inset-0 flex items-center" aria-hidden="true">
                  <div className="w-[90%] border-t border-slate-400 m-auto" />
                </div>
                <div className="relative flex justify-center">
                  <span className="bg-slate-100 mb-4 px-3 text-xl font-semibold leading-6 text-slate-800">Candidate Answers</span>
                </div>
              </div>
              {
                quizTaken.answers && quizTaken.answers.map((answer, index) => (
                  <div key={index} className='flex items-center space-x-2 justify-center'>
                    <p className=''>Answer {index + 1}: <span className='font-semibold'>{answer.answer}</span></p>
                  </div>
                ))
              }
              <div className="relative mt-5">
