    ...
    const fetchQuizData = async () => {
      ...
      try {
        const { data } = await axios.get(
          `https://software-programming-quiz-api.onrender.com/quiz/${id}`,
          config
        );
        setQuiz(data.quiz);
      } catch (error) {
          ...
          <div className="bg-slate-100 rounded-md shadow-lg py-8 w-full text-lg">
            <div className="text-center">
              <Link to={`/quiz-results/${id}`}>
                <button className="bg-slate-500 border-2 border-slate-700 text-slate-50 w-11/12 px-4 py-2 rounded-full shadow-md hover:bg-slate-600 transition duration-300 mb-5">
                  View Quiz Results
                </button>
              </Link>
