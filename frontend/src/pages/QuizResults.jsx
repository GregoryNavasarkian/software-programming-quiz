...
const QuizResults = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        const { data } = await axios.get(`http://localhost:5000/candidate/${id}/taken`, config);
  ...
  return (
    <div className='w-full min-h-screen py-16 px-4 shadow-lg bg-slate-200 mt-20'>
      <div className='max-w-[1250px] mx-auto'>
        <h1 className='md:text-4xl text-3xl font-semibold text-slate-800 mt-2 md:text-left text-center'>Results</h1>
        <p className='text-slate-900 text-lg mt-1'>Note: Sort Answer Questions not counted in score.</p>
            ...
            <div className='text-center'>
              {
                ...
                <div>
                  ...
                  <h3 className="mt-2 text-base font-semibold text-slate-900">Quiz not taken by any candidates</h3>
                </div>
              }
              {
                isQuizTaken && takenQuizzes.map((quiz, index) => (
